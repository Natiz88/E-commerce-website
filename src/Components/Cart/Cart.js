    ...
    <div className='cart__container'>
        <div className="cart__items">{
          cart.map(product=>
            ...
            <h2>Dish: <span>{product.rname}</span></h2>
            <h2>Ratings: <span>{product.rating} ⭐</span></h2>
            <h2>Price: <span>Rs.{product.price}</span></h2>
            <h2>Quantity: <span>
              <button onClick={() => dispatch(actionTypes.subQty(product.id))}>-</button>
                {product.qty}
              <button onClick={() => dispatch(actionTypes.addQty(product.id))}>+</button></span></h2>
            <div className="buttons">
          <button onClick={() => dispatch(actions.addToList(product.id))}><FaHeart className="heart" /></button>
          <button onClick={() => dispatch(actionTypes.removeFromCart(product.id))}>Remove From Cart</button>
        </div>
        </div>
        ...
        <div className="checkout">
            ...
            <h3>Total MRP<span>{totalMRP}</span></h3>
